    ...
    if (target.tagName === 'IMG' && target.src) {
        ...
        hoverTimeout = setTimeout(() => {
            ...
            chrome.runtime.sendMessage({
                ...
                const usageFooter = usageInfo && typeof usageInfo.freeTooltipsRemaining === 'number'
                    ? `<p class="ai-tooltip-footer">Free tooltips left: ${usageInfo.freeTooltipsRemaining}/${usageInfo.freeTierLimit}</p>`
                    : '';

                if (response && response.success) {
                    showTooltip(target, `<p><strong>Image OCR Result:</strong></p><p>${response.result}</p>${usageFooter}`);
                } else if (response && response.error) {
                    const upgradeHint = response.errorCode === 'FREE_TIER_EXHAUSTED'
                        ? `<p><em>Limit reached. Open the extension popup to upgrade.</em></p>`
                        : '';
                    showTooltip(target, `<p><strong>Error:</strong></p><p>${response.error}</p>${upgradeHint}`);
                } else {
                    showTooltip(target, `<p><strong>Error:</strong></p><p>Failed to get response from background worker.</p>`);
                }
            });
        }, HOVER_DELAY);
    }
